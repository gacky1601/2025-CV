...
from Car_Detection_TF.yolo import YOLO   # install yolo-v3 model (後加)
...
class JsonDecoder(threading.Thread):
    ...
    def detect(self,camera_id,starting_frame_id,frames,frame_width,frame_height,read_file,boxes_file,city,district_no, save_txt_path=None):
        start_detect_time = time()
        # if not read_file and self.yolo == None:
        # self.yolo = YOLO()

        detection = Detection(self.yolo)
        # print("YOLO is seu up successfully!")
        boxes = detection.detect(frames, frame_width, frame_height, read_file, boxes_file, self.read_file, self.tf, save_txt_path=save_txt_path)
        print("Detect",camera_id, " Frame: ", starting_frame_id+len(frames))
        print("*"*10, " Start ", "*"*10)
        print(boxes)
        print("*"*10, " End ", "*"*10)
        print('\n')
